...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountBalances({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  page: 7694, // ページ番号
  per_page: 6358, // 1ページ分の取引数
  expires_at_from: "2020-07-18T13:15:28.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2026-03-28T22:41:39.000000Z", // 有効期限の期間によるフィルター(終了時点)
...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountExpiredBalances({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  page: 5253, // ページ番号
  per_page: 9707, // 1ページ分の取引数
  expires_at_from: "2023-01-12T22:39:56.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2026-02-14T16:06:10.000000Z", // 有効期限の期間によるフィルター(終了時点)
  direction: "asc" // 有効期限によるソート順序
}));
```
...
const response: Response<AccountWithUser> = await client.send(new UpdateCustomerAccount({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  status: "active", // ウォレット状態
  account_name: "KaUCU4cuncfOgZgC0vnz9vdHX3zI21M9POKUqkrXtAeLmERqX5bwDROtzb2hizqeaCyQXA4kt1s5IzgftNOCei", // アカウント名
  external_id: "OWbpouk4VaYSYsKX6oU", // 外部ID
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetCustomerAccounts({
  ...
  page: 9825, // ページ番号
  per_page: 5918, // 1ページ分のウォレット数
  created_at_from: "2025-12-16T17:47:56.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2022-11-19T01:43:16.000000Z", // ウォレット作成日によるフィルター(終了時点)
  is_suspended: true, // ウォレットが凍結状態かどうかでフィルターする
  status: "pre-closed", // ウォレット状態
  external_id: "fTNsJ74FdhPrGorQztiuURWZ5r1OnryKkdpm", // 外部ID
  tel: "066947-936", // エンドユーザーの電話番号
  email: "cgSjEKEvn9@tkKJ.com" // エンドユーザーのメールアドレス
}));
```
...
const response: Response<AccountWithUser> = await client.send(new CreateCustomerAccount({
  ...
  external_id: "sfEeEirDJBvMOLUpWvpkfaBwAHAugbJ1Kgm" // 外部ID
}));
```
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetShopAccounts({
  private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
  page: 9297, // ページ番号
  per_page: 9582, // 1ページ分のウォレット数
  created_at_from: "2022-03-18T20:07:00.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2022-07-08T14:45:11.000000Z", // ウォレット作成日によるフィルター(終了時点)
...
const response: Response<PaginatedTransaction> = await client.send(new ListCustomerTransactions({
  ...
  from: "2023-09-13T09:54:16.000000Z", // 開始日時
  to: "2025-12-31T13:52:49.000000Z", // 終了日時
  page: 1, // ページ番号
