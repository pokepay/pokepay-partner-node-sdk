...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountBalances({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  page: 9516, // ページ番号
  per_page: 2657, // 1ページ分の取引数
  expires_at_from: "2023-12-27T17:24:01.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2023-11-21T04:04:04.000000Z", // 有効期限の期間によるフィルター(終了時点)
...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountExpiredBalances({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  page: 3757, // ページ番号
  per_page: 9193, // 1ページ分の取引数
  expires_at_from: "2024-08-29T14:23:06.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2024-06-25T23:49:16.000000Z", // 有効期限の期間によるフィルター(終了時点)
  direction: "desc" // 有効期限によるソート順序
}));
```
...
const response: Response<AccountWithUser> = await client.send(new UpdateCustomerAccount({
  ...
  account_name: "1yqyLKdrb7VdvBferrdPPsgFTBp21GVpuNthlN8cTNxtClPPAh3ydu7juMaO7kqGjaASQkqyw2Q45pim16jWY8Li2yJuAILC9WmiQzTAP0hsvYk94ECXfwyrT6FNWSeiPJDkaNGUUFy37fVBCxguWkgEaSRxikajDhky1e9MUM8ZY9eEBDTjFI18oRpgCoDiEOfsuO3LM", // アカウント名
  external_id: "zPm5pmHiztzTLcjSeN", // 外部ID
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetCustomerAccounts({
  ...
  page: 7526, // ページ番号
  per_page: 4080, // 1ページ分のウォレット数
  created_at_from: "2021-01-10T15:28:15.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2021-11-18T11:48:04.000000Z", // ウォレット作成日によるフィルター(終了時点)
  is_suspended: false, // ウォレットが凍結状態かどうかでフィルターする
  status: "pre-closed", // ウォレット状態
  external_id: "1SbLY9f9RM3h2SXQaAm6iMSYVoPQWfV62", // 外部ID
  tel: "084-43019166", // エンドユーザーの電話番号
  email: "OsA2Q2Z23M@wd98.com" // エンドユーザーのメールアドレス
}));
```
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetShopAccounts({
  private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
  page: 6884, // ページ番号
  per_page: 8706, // 1ページ分のウォレット数
  created_at_from: "2020-10-23T02:38:04.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2020-06-10T16:10:02.000000Z", // ウォレット作成日によるフィルター(終了時点)
...
const response: Response<PaginatedTransaction> = await client.send(new ListCustomerTransactions({
  ...
  from: "2021-06-28T03:48:33.000000Z", // 開始日時
  to: "2020-10-04T07:24:43.000000Z", // 終了日時
  page: 1, // ページ番号
