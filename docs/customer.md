...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountBalances({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  page: 817, // ページ番号
  per_page: 6232, // 1ページ分の取引数
  expires_at_from: "2024-07-07T23:03:59.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2023-08-28T12:22:34.000000Z", // 有効期限の期間によるフィルター(終了時点)
...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountExpiredBalances({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  page: 90, // ページ番号
  per_page: 5114, // 1ページ分の取引数
  expires_at_from: "2022-05-13T07:15:30.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2025-06-15T15:55:11.000000Z", // 有効期限の期間によるフィルター(終了時点)
...
const response: Response<AccountWithUser> = await client.send(new UpdateCustomerAccount({
  ...
  account_name: "5d5RKAlbrPQ0st0t7yJcv8GqBqgGEHafl1jNP9k7uydClg9A7an27PrVx", // アカウント名
  external_id: "qiE", // 外部ID
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetCustomerAccounts({
  ...
  page: 3498, // ページ番号
  per_page: 4634, // 1ページ分のウォレット数
  created_at_from: "2022-12-07T16:06:34.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2026-03-02T23:29:27.000000Z", // ウォレット作成日によるフィルター(終了時点)
  is_suspended: false, // ウォレットが凍結状態かどうかでフィルターする
  status: "suspended", // ウォレット状態
  external_id: "o8xjmzBGJVwTTanAXyFjLag3gPPvlq0FFn", // 外部ID
  tel: "04192-1109", // エンドユーザーの電話番号
  email: "p27NPGQTdA@XKNG.com" // エンドユーザーのメールアドレス
}));
```
...
const response: Response<AccountWithUser> = await client.send(new CreateCustomerAccount({
  ...
  external_id: "uLNgDO4Ma1ptA22IkyjkgPuZUMAq2NjJocN" // 外部ID
}));
```
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetShopAccounts({
  private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
  page: 8854, // ページ番号
  per_page: 90, // 1ページ分のウォレット数
  created_at_from: "2024-11-06T06:24:01.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2022-05-21T23:56:59.000000Z", // ウォレット作成日によるフィルター(終了時点)
...
const response: Response<PaginatedTransaction> = await client.send(new ListCustomerTransactions({
  ...
  from: "2023-10-21T16:33:53.000000Z", // 開始日時
  to: "2024-03-10T02:29:40.000000Z", // 終了日時
  page: 1, // ページ番号
