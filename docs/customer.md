...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountBalances({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  page: 5208, // ページ番号
  per_page: 3366, // 1ページ分の取引数
  expires_at_from: "2022-02-11T22:20:04.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2022-11-09T06:48:24.000000Z", // 有効期限の期間によるフィルター(終了時点)
...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountExpiredBalances({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  page: 3148, // ページ番号
  per_page: 1007, // 1ページ分の取引数
  expires_at_from: "2026-04-22T05:40:49.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2020-11-03T14:02:26.000000Z", // 有効期限の期間によるフィルター(終了時点)
  direction: "asc" // 有効期限によるソート順序
}));
```
...
const response: Response<AccountWithUser> = await client.send(new UpdateCustomerAccount({
  ...
  account_name: "oT8w3801ZxeZpTa0FFkkUFLVCDKp9TvCsVFg3Dy6t9FVfvRBKOl2QQeBI5NM6J7EhkzGk22yYle2ZOPXJOiEYcNwwBKhoxCdqw8SDS6L7O6ohLm8HBuYz7E9ZuYBAHz0vH45u4SHdXpfYeqMtcfd8wxcygI", // アカウント名
  external_id: "1kAzyAHjkW0eFslSf8NaBTyV", // 外部ID
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetCustomerAccounts({
  ...
  page: 8201, // ページ番号
  per_page: 5955, // 1ページ分のウォレット数
  created_at_from: "2022-09-08T11:40:05.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2021-02-26T09:07:32.000000Z", // ウォレット作成日によるフィルター(終了時点)
  is_suspended: true, // ウォレットが凍結状態かどうかでフィルターする
  status: "active", // ウォレット状態
  external_id: "HI0zW", // 外部ID
  tel: "041-320-0549", // エンドユーザーの電話番号
  email: "HOOwl5xIQi@AP4U.com" // エンドユーザーのメールアドレス
}));
```
...
const response: Response<AccountWithUser> = await client.send(new CreateCustomerAccount({
  ...
  external_id: "lfuFUQK5yc0JqyEbk4xV1ElwOVpwOgCs3REJLXlOpH9qH3Tnt" // 外部ID
}));
```
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetShopAccounts({
  private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
  page: 4989, // ページ番号
  per_page: 8614, // 1ページ分のウォレット数
  created_at_from: "2024-09-27T08:52:44.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2023-09-15T08:40:24.000000Z", // ウォレット作成日によるフィルター(終了時点)
  is_suspended: true // ウォレットが凍結状態かどうかでフィルターする
}));
```
...
const response: Response<PaginatedTransaction> = await client.send(new ListCustomerTransactions({
  ...
  from: "2025-12-16T18:15:36.000000Z", // 開始日時
  to: "2020-08-15T23:08:11.000000Z", // 終了日時
  page: 1, // ページ番号
