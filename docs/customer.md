...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountBalances({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  page: 4940, // ページ番号
  per_page: 7290, // 1ページ分の取引数
  expires_at_from: "2022-10-04T16:44:38.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2025-04-21T09:13:24.000000Z", // 有効期限の期間によるフィルター(終了時点)
...
const response: Response<PaginatedAccountBalance> = await client.send(new ListAccountExpiredBalances({
  ...
  expires_at_from: "2021-07-02T13:56:18.000000Z", // 有効期限の期間によるフィルター(開始時点)
  expires_at_to: "2021-08-24T04:53:34.000000Z", // 有効期限の期間によるフィルター(終了時点)
...
const response: Response<AccountWithUser> = await client.send(new UpdateCustomerAccount({
  account_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ウォレットID
  status: "suspended", // ウォレット状態
  account_name: "5kIXisF2lvLdWFAH9CECfmZyvOgcw2bcIoYI3B409EBsOM5mHn7CA1SM3xNEFCgQheyCbSnP7P0SqnjQBF0gNpyvaBHzjlAdXU9fbl4BElEfYJcTmiRof0lbldCRsSSTgoxqh3aCnDQum7xlHp8mSoN73gaH3XPjunt8NgffostplBJ13qPcXVXQ9E7OqefuC0zsB8aQbgel1VXLZNhM7VCGf", // アカウント名
  external_id: "zH0EqAidHGV4baZPNRUSJ9iQ", // 外部ID
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetCustomerAccounts({
  ...
  page: 9935, // ページ番号
  per_page: 1807, // 1ページ分のウォレット数
  created_at_from: "2025-08-07T02:02:42.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2021-07-08T15:40:50.000000Z", // ウォレット作成日によるフィルター(終了時点)
  is_suspended: false, // ウォレットが凍結状態かどうかでフィルターする
  status: "suspended", // ウォレット状態
  external_id: "hlAuhO2DrrEN6v7h6D", // 外部ID
  tel: "09820300782", // エンドユーザーの電話番号
  email: "JykFEWCqS7@fIGs.com" // エンドユーザーのメールアドレス
}));
```
...
const response: Response<AccountWithUser> = await client.send(new CreateCustomerAccount({
  ...
  external_id: "SUetvzhcyY8O4aW8dVGclxW2nJI1LDT3BhMLUADb" // 外部ID
}));
```
...
const response: Response<PaginatedAccountWithUsers> = await client.send(new GetShopAccounts({
  private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
  page: 3975, // ページ番号
  per_page: 6940, // 1ページ分のウォレット数
  created_at_from: "2023-02-03T05:43:49.000000Z", // ウォレット作成日によるフィルター(開始時点)
  created_at_to: "2021-09-11T18:50:02.000000Z", // ウォレット作成日によるフィルター(終了時点)
...
const response: Response<PaginatedTransaction> = await client.send(new ListCustomerTransactions({
  ...
  from: "2022-09-13T14:21:24.000000Z", // 開始日時
  to: "2022-12-28T00:41:04.000000Z", // 終了日時
  page: 1, // ページ番号
