...
const response: Response<PaginatedCampaigns> = await client.send(new ListCampaigns({
  ...
  available_from: "2023-01-03T18:36:35.000000Z", // 指定された日時以降に適用可能期間が含まれているか
  available_to: "2020-06-15T22:53:40.000000Z", // 指定された日時以前に適用可能期間が含まれているか
  page: 1, // ページ番号
...
const response: Response<Campaign> = await client.send(new CreateCampaign({
  name: "lxmPSv0sqeMHVeJGZnQaE4lp3S7TMyfZKpPybiZ1Lwce18e7Eq5OqWuTabdRaaHOyfGqVUncXzhjskeGyZxmbEy050Zlv3tzVr8aTPDqMKbxS0Vs3OlIrdnx7rU9F", // キャンペーン名
  private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
  starts_at: "2020-04-02T06:14:00.000000Z", // キャンペーン開始日時
  ends_at: "2024-09-07T02:24:52.000000Z", // キャンペーン終了日時
  priority: 6329, // キャンペーンの適用優先度
  event: "external-transaction", // イベント種別
  bear_point_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ポイント負担先店舗ID
  description: "59oBy13mtel3d8TfJ3Ol39ScasZnA58jo0hnztlMdM7BVfn4iFYyJJXfrDUn2Z5dTBMhYMOaLFSQqsldJHk3l4cpZ7fJl29A3O6y0fQnXOgwkIth5yMWiTVYzb9YasuIp7v4EzACicWq4Ul0bBBFnJwjrPufrwL5Z4qM5cyeftMbZhJuNsCdqVbAgL", // キャンペーンの説明文
  status: "enabled", // キャンペーン作成時の状態
  point_expires_at: "2020-05-06T17:28:49.000000Z", // ポイント有効期限(絶対日時指定)
  point_expires_in_days: 4940, // ポイント有効期限(相対日数指定)
  ...
  minimum_number_of_products: 2913, // キャンペーンを適用する1会計内の商品個数の下限
  minimum_number_of_amount: 7212, // キャンペーンを適用する1会計内の商品総額の下限
  minimum_number_for_combination_purchase: 3557, // 複数種類の商品を同時購入するときの商品種別数の下限
  exist_in_each_product_groups: false, // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
  max_point_amount: 8075, // キャンペーンによって付与されるポイントの上限
  max_total_point_amount: 4358, // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  ...
  budget_caps_amount: 204895277 // キャンペーン予算上限
}));
```
...
const response: Response<Campaign> = await client.send(new UpdateCampaign({
  campaign_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // キャンペーンID
  name: "VC", // キャンペーン名
  starts_at: "2024-09-21T13:26:46.000000Z", // キャンペーン開始日時
  ends_at: "2023-10-11T17:32:35.000000Z", // キャンペーン終了日時
  priority: 5816, // キャンペーンの適用優先度
  event: "topup", // イベント種別
  description: "MOaKHSf5htPpycWdWsbduWBxtfg1Kliu47KITpvwbo61t0xPHohZAfXS5WAq97VI0kJjyO9S00lRKqhRSKyv4aeUNiX5kIXisF2lvLdWFAH9CECfmZy", // キャンペーンの説明文
  status: "disabled", // キャンペーン作成時の状態
  point_expires_at: "2024-02-16T07:02:14.000000Z", // ポイント有効期限(絶対日時指定)
  point_expires_in_days: 2920, // ポイント有効期限(相対日数指定)
  is_exclusive: true, // キャンペーンの重複設定
  ...
  applicable_days_of_week: [3, 3, 2], // キャンペーンを適用する曜日 (複数指定)
  applicable_time_ranges: [{
  "from": "12:00",
  "to": "23:59"
}, {
  ...
  minimum_number_of_products: 9880, // キャンペーンを適用する1会計内の商品個数の下限
  minimum_number_of_amount: 5548, // キャンペーンを適用する1会計内の商品総額の下限
  minimum_number_for_combination_purchase: 9862, // 複数種類の商品を同時購入するときの商品種別数の下限
  exist_in_each_product_groups: true, // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
  max_point_amount: 703, // キャンペーンによって付与されるポイントの上限
  max_total_point_amount: 692, // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  applicable_account_metadata: {
  "key": "sex",
  "value": "male"
}, // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
  ...
  budget_caps_amount: 179686979 // キャンペーン予算上限
}));
```
