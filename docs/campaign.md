...
const response: Response<PaginatedCampaigns> = await client.send(new ListCampaigns({
  ...
  available_from: "2023-03-15T18:08:06.000000Z", // 指定された日時以降に適用可能期間が含まれているか
  available_to: "2021-07-22T01:16:05.000000Z", // 指定された日時以前に適用可能期間が含まれているか
  page: 1, // ページ番号
...
const response: Response<Campaign> = await client.send(new CreateCampaign({
  name: "HHeXuuwg12Ygg3AsTOryINKyRmJ3gWCDcmsuvkMrJePtGFhv4aIw1aGtGR3fEQezBo8XnXONHGXD", // キャンペーン名
  private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
  starts_at: "2021-11-14T03:38:15.000000Z", // キャンペーン開始日時
  ends_at: "2021-07-26T06:32:03.000000Z", // キャンペーン終了日時
  priority: 3343, // キャンペーンの適用優先度
  event: "topup", // イベント種別
  bear_point_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ポイント負担先店舗ID
  description: "tuhVdB5KkP8PHvZEmmcBKkGsr9sdEDTBkey7pr4d2jpaf36YY6mrG9Y2z", // キャンペーンの説明文
  status: "enabled", // キャンペーン作成時の状態
  point_expires_at: "2021-01-13T17:51:58.000000Z", // ポイント有効期限(絶対日時指定)
  point_expires_in_days: 9619, // ポイント有効期限(相対日数指定)
  ...
  minimum_number_of_products: 1301, // キャンペーンを適用する1会計内の商品個数の下限
  minimum_number_of_amount: 1805, // キャンペーンを適用する1会計内の商品総額の下限
  minimum_number_for_combination_purchase: 1458, // 複数種類の商品を同時購入するときの商品種別数の下限
  exist_in_each_product_groups: true, // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
  max_point_amount: 1668, // キャンペーンによって付与されるポイントの上限
  max_total_point_amount: 5844, // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  ...
  budget_caps_amount: 54420432 // キャンペーン予算上限
}));
```
...
const response: Response<Campaign> = await client.send(new UpdateCampaign({
  campaign_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // キャンペーンID
  name: "xEgnoe60dnWTCVmm3x115QsBZT6dCGgqZsePkl6iY0bdXM6Nza2rTctUJQmh0gNd3qkWY4lVW5zCUF3zWzIdrHm6OsiyHBxsWBtx4G7cLViMByCBNzcDCX5bbsPzVUGeD2BWp2XUNEsAtEjlivj0NhalsavWYZduuXynvh05rJdAnnKPkjJzRbGyu", // キャンペーン名
  starts_at: "2022-08-17T17:20:25.000000Z", // キャンペーン開始日時
  ends_at: "2020-02-01T08:18:42.000000Z", // キャンペーン終了日時
  priority: 57, // キャンペーンの適用優先度
  event: "payment", // イベント種別
  description: "8tP6VkRaNaNdjmk2wkclkjGIdrGdF8qpLKYfd3JbJX5QcdKyJ1DmsToKu4w1tRUaP7awM87Mt7bWysOyzqkBrGaMjb1sugqjEeek3DeIDBfKsRBbYLkU2TfJXzuBqGFPReFsmxaxT8Xwuc649dznjsqwxML0aHpiMuFL917lUTrE8EACTMWkW", // キャンペーンの説明文
  status: "disabled", // キャンペーン作成時の状態
  point_expires_at: "2022-02-04T10:38:43.000000Z", // ポイント有効期限(絶対日時指定)
  point_expires_in_days: 783, // ポイント有効期限(相対日数指定)
  ...
  minimum_number_of_products: 4048, // キャンペーンを適用する1会計内の商品個数の下限
  minimum_number_of_amount: 6832, // キャンペーンを適用する1会計内の商品総額の下限
  minimum_number_for_combination_purchase: 6184, // 複数種類の商品を同時購入するときの商品種別数の下限
  exist_in_each_product_groups: true, // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
  max_point_amount: 3505, // キャンペーンによって付与されるポイントの上限
  max_total_point_amount: 817, // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  applicable_account_metadata: {
  "key": "sex",
  "value": "male"
}, // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
  ...
  budget_caps_amount: 527128664 // キャンペーン予算上限
}));
```
