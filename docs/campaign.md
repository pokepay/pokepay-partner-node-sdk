...
const response: Response<PaginatedCampaigns> = await client.send(new ListCampaigns({
  ...
  available_from: "2021-11-22T09:02:08.000000Z", // 指定された日時以降に適用可能期間が含まれているか
  available_to: "2023-08-28T05:18:48.000000Z", // 指定された日時以前に適用可能期間が含まれているか
  page: 1, // ページ番号
...
const response: Response<Campaign> = await client.send(new CreateCampaign({
  name: "1owGN6i0XTT33lqYdKQ0h3ghVZk7eOE9tcwx8MOKl5M", // キャンペーン名
  private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
  starts_at: "2026-07-02T05:34:55.000000Z", // キャンペーン開始日時
  ends_at: "2021-01-18T07:20:15.000000Z", // キャンペーン終了日時
  priority: 5036, // キャンペーンの適用優先度
  event: "external-transaction", // イベント種別
  bear_point_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ポイント負担先店舗ID
  description: "1MFEYPOVzvPSXDUkbgX2oBshUtXGZ9lfp9TwgYPOmismihXWyqdhqoMR6oAdT5yPsPRTmUYdZdYDDGZDuZn0XgqQIqTu14tSh1", // キャンペーンの説明文
  status: "disabled", // キャンペーン作成時の状態
  point_expires_at: "2020-09-10T12:06:19.000000Z", // ポイント有効期限(絶対日時指定)
  point_expires_in_days: 1778, // ポイント有効期限(相対日数指定)
  ...
  applicable_days_of_week: [4, 2], // キャンペーンを適用する曜日 (複数指定)
  ...
  minimum_number_of_products: 2339, // キャンペーンを適用する1会計内の商品個数の下限
  minimum_number_of_amount: 5149, // キャンペーンを適用する1会計内の商品総額の下限
  minimum_number_for_combination_purchase: 3643, // 複数種類の商品を同時購入するときの商品種別数の下限
  exist_in_each_product_groups: true, // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
  max_point_amount: 893, // キャンペーンによって付与されるポイントの上限
  max_total_point_amount: 995, // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  ...
  budget_caps_amount: 1432323534 // キャンペーン予算上限
}));
```
...
const response: Response<Campaign> = await client.send(new UpdateCampaign({
  campaign_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // キャンペーンID
  name: "gZiB4q5yXIKvcyeytZUeCOzn479Q7e7CQ6mogsi4OQ6jQwMdVQzET3CTZR3naadmHoO937wRncWgLEMvwuXtyGneCNJhR9grzsET9HHziGJ2iqEYWh5QfKEnNvZa51B6RuNHWw3kkEIImb7", // キャンペーン名
  starts_at: "2023-09-25T12:05:17.000000Z", // キャンペーン開始日時
  ends_at: "2020-04-08T08:49:23.000000Z", // キャンペーン終了日時
  priority: 8760, // キャンペーンの適用優先度
  event: "payment", // イベント種別
  description: "g0GpEoXRZP9Tuo6ihkLtNpmjVgJl2arbhJouxWQ6FlBm7k1iTzlm9ILQGKVJoUCSY35cdkgvsbAYCbaEHjTHUmx8bpMxYByLz0xsJRhRVsB9HjzBAZfWzO75yHWR5FLMa9CO3GmqQepv7doxpRjgZI2VSDvLJkkZMMdEAN", // キャンペーンの説明文
  status: "enabled", // キャンペーン作成時の状態
  point_expires_at: "2021-09-11T00:32:00.000000Z", // ポイント有効期限(絶対日時指定)
  point_expires_in_days: 1495, // ポイント有効期限(相対日数指定)
  is_exclusive: true, // キャンペーンの重複設定
  subject: "money", // ポイント付与の対象金額の種別
  amount_based_point_rules: [{
  "point_amount": 5,
  ...
  blacklisted_shop_ids: ["xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"], // キャンペーン適用対象外となる店舗IDのリスト(ブラックリスト方式)
  minimum_number_of_products: 1617, // キャンペーンを適用する1会計内の商品個数の下限
  minimum_number_of_amount: 3735, // キャンペーンを適用する1会計内の商品総額の下限
  minimum_number_for_combination_purchase: 971, // 複数種類の商品を同時購入するときの商品種別数の下限
  exist_in_each_product_groups: false, // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
  max_point_amount: 447, // キャンペーンによって付与されるポイントの上限
  max_total_point_amount: 3732, // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  applicable_account_metadata: {
  "key": "sex",
  "value": "male"
}, // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
  ...
  budget_caps_amount: 1609899229 // キャンペーン予算上限
}));
```
