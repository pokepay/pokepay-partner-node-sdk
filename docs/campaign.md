...
const response: Response<PaginatedCampaigns> = await client.send(new ListCampaigns({
  ...
  available_from: "2022-05-17T13:58:51.000000Z", // 指定された日時以降に適用可能期間が含まれているか
  available_to: "2024-02-10T14:28:57.000000Z", // 指定された日時以前に適用可能期間が含まれているか
  page: 1, // ページ番号
...
const response: Response<Campaign> = await client.send(new CreateCampaign({
  name: "W2ZGwTADg0EGo2tY0BvAArU4c3Hcr3rYtMZs1YhEQlphw1DkmThPoIdPA7X1r8JTPyIk7mw82VAIRkHcNMgqN77FQwuiGtQW4pnFSkfz0ZAYuHKErS89ga8rAwXpAiqwTxt1HL", // キャンペーン名
  private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
  starts_at: "2024-06-15T16:54:12.000000Z", // キャンペーン開始日時
  ends_at: "2025-08-05T23:47:53.000000Z", // キャンペーン終了日時
  priority: 1399, // キャンペーンの適用優先度
  event: "topup", // イベント種別
  bear_point_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ポイント負担先店舗ID
  description: "WzmkMDA4SVfWD13Zj3L9DQPYajb0tVdWE", // キャンペーンの説明文
  status: "enabled", // キャンペーン作成時の状態
  point_expires_at: "2021-10-19T18:48:04.000000Z", // ポイント有効期限(絶対日時指定)
  point_expires_in_days: 7052, // ポイント有効期限(相対日数指定)
  ...
  applicable_days_of_week: [5, 0, 1], // キャンペーンを適用する曜日 (複数指定)
  ...
  minimum_number_of_products: 4553, // キャンペーンを適用する1会計内の商品個数の下限
  minimum_number_of_amount: 3384, // キャンペーンを適用する1会計内の商品総額の下限
  minimum_number_for_combination_purchase: 393, // 複数種類の商品を同時購入するときの商品種別数の下限
  exist_in_each_product_groups: false, // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
  max_point_amount: 8874, // キャンペーンによって付与されるポイントの上限
  max_total_point_amount: 945, // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  ...
  budget_caps_amount: 1593120803 // キャンペーン予算上限
}));
```
...
const response: Response<Campaign> = await client.send(new UpdateCampaign({
  campaign_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // キャンペーンID
  name: "c9iXi2Q1VWdznJovLhT0BrHHw3tEdBOJZocfpIFBg2EP1IMpzVlOR0ZjHbJ4pIYeH1mIjK91BovJNiyan2Rg9xEgMUhIRyB0Lq7z8Ljil9JSMA7rA7mkLLtmKfguDK2IgQ", // キャンペーン名
  starts_at: "2020-01-23T23:31:54.000000Z", // キャンペーン開始日時
  ends_at: "2021-06-15T23:35:43.000000Z", // キャンペーン終了日時
  priority: 2473, // キャンペーンの適用優先度
  event: "external-transaction", // イベント種別
  description: "DYIDOJbPEulQIvNSkQALktsxpQN", // キャンペーンの説明文
  status: "enabled", // キャンペーン作成時の状態
  point_expires_at: "2025-02-05T11:38:30.000000Z", // ポイント有効期限(絶対日時指定)
  point_expires_in_days: 7479, // ポイント有効期限(相対日数指定)
  is_exclusive: true, // キャンペーンの重複設定
  ...
  applicable_days_of_week: [5, 0, 4], // キャンペーンを適用する曜日 (複数指定)
  applicable_time_ranges: [{
  "from": "12:00",
  "to": "23:59"
}], // キャンペーンを適用する時間帯 (複数指定)
  ...
  minimum_number_of_products: 8047, // キャンペーンを適用する1会計内の商品個数の下限
  minimum_number_of_amount: 2096, // キャンペーンを適用する1会計内の商品総額の下限
  minimum_number_for_combination_purchase: 5413, // 複数種類の商品を同時購入するときの商品種別数の下限
  exist_in_each_product_groups: true, // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
  max_point_amount: 5587, // キャンペーンによって付与されるポイントの上限
  max_total_point_amount: 2830, // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  applicable_account_metadata: {
  "key": "sex",
  "value": "male"
}, // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
  ...
  budget_caps_amount: 735974902 // キャンペーン予算上限
}));
```
